...
function handleWindowResize() {
    ...
    applyReshape(fullpage, window.innerWidth, (window.innerHeight*0.80));
}
...
function sendText(e) {
    ...
    if( stringToSend && stringToSend != "") {
        ...
        chatArea.append("Me:\t"+stringToSend+"\n\t\t");
        rescrollChatArea();
    }
    return false;
}
...
function showTextEntry() {
    ...
    document.getElementById('textentryField').focus();
    document.getElementById('textentryField').reset();
}


function showMessage(startX, startY, content, easyrtcid) {
    var fullPage = document.getElementById('fullpage');
    ...
    var chatArea = document.getElementById('chatArea');
    chatArea.append(easyrtcid+":\t"+content+"\n\t\t");
    rescrollChatArea();
...
function messageListener(easyrtcid, msgType, content) {
    for(var i = 0; i < maxCALLERS; i++) {
        if( easyrtc.getIthCaller(i) == easyrtcid) {
            ...
            showMessage(startX, startY, content, easyrtcid);
        }
    }
}
...
function appInit() {
    ...
    easyrtc.setOnCall( function(easyrtcid, slot) {
        ...
        handleWindowResize();
        if  (myrtcid==easyrtcid)
            document.getElementById('chatArea').append("I joined.\n\t\t");
        else
            document.getElementById('chatArea').append(easyrtcid+" joined.\n\t\t");
        rescrollChatArea();
    });
    ...
    easyrtc.setOnHangup(function(easyrtcid, slot) {
        ...
        setTimeout(function() {
            ...
            if( easyrtc.getConnectionCount() == 0 ) { // no more connections
                expandThumb(0);
                if  (myrtcid==easyrtcid)
                    document.getElementById('chatArea').append("I exited.\n\t\t");
                else
                    document.getElementById('chatArea').append(easyrtcid+" exited.\n\t\t");
                rescrollChatArea();
                /*document.getElementById('textEntryButton').style.display = 'none';*/
                /*document.getElementById('textentryBox').style.display = 'none';*/
            }
...
function startRecording() {
  ...
  var callerLink1 = document.getElementById("callerDownloadLink1");
  callerLink1.innerText = "";
  if( easyrtc.getIthCaller(0)) {
     callerRecorder1 = easyrtc.recordToFile(
       easyrtc.getRemoteStream(easyrtc.getIthCaller(0), null), 
       callerLink1, "caller1Video");
     if( !callerRecorder1 ) {
      window.alert("failed to start recorder for caller 1");
     }
  }else {
     callerRecorder1 = null;
  }
  
  var callerLink2 = document.getElementById("callerDownloadLink2");
  callerLink2.innerText = "";
  if( easyrtc.getIthCaller(1)) {
     callerRecorder2 = easyrtc.recordToFile(
       easyrtc.getRemoteStream(easyrtc.getIthCaller(1), null), 
       callerLink2, "caller2Video");
     if( !callerRecorder2 ) {
      window.alert("failed to start recorder for caller2");
     }
  }else {
     callerRecorder2 = null;
  }
  
  var callerLink3 = document.getElementById("callerDownloadLink3");
  callerLink3.innerText = "";
  if( easyrtc.getIthCaller(2)) {
     callerRecorder3 = easyrtc.recordToFile(
       easyrtc.getRemoteStream(easyrtc.getIthCaller(2), null), 
       callerLink3, "caller3Video");
     if( !callerRecorder3 ) {
      window.alert("failed to start recorder for caller3");
     }
  }else {
     callerRecorder3 = null;
  }
  
  var callerLink4 = document.getElementById("callerDownloadLink4");
  callerLink4.innerText = "";
  if( easyrtc.getIthCaller(3)) {
     callerRecorder4 = easyrtc.recordToFile(
       easyrtc.getRemoteStream(easyrtc.getIthCaller(3), null), 
       callerLink4, "caller4Video");
     if( !callerRecorder4 ) {
      window.alert("failed to start recorder for caller4");
     }
  }else {
     callerRecorder4 = null;
  }
  
  document.getElementById("linkToFiles").style.display = "none";
  
  var stringToSend = " -- Started Recording --";
  for (var i = 0; i < maxCALLERS; i++ ) {
      var easyrtcid = easyrtc.getIthCaller(i);
      if( easyrtcid && easyrtcid != "") {
          easyrtc.sendPeerMessage(easyrtcid, "im",  stringToSend);
...
function endRecording() {
  ...
  if( callerRecorder1 ) {
     callerRecorder1.stop();
  }
  if( callerRecorder2 ) {
     callerRecorder2.stop();
  }
  if( callerRecorder3 ) {
     callerRecorder3.stop();
  }
  if( callerRecorder4 ) {
     callerRecorder4.stop();
  }
  document.getElementById("startRecording").disabled = false;
  document.getElementById("stopRecording").disabled = true;
  document.getElementById("linkToFiles").style.display = "block";
}
